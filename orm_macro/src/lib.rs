    ...
    let expanded = quote! {
        impl #name {
            pub async fn create_schema(session: &scylla::Session) -> crate::result::Result<()> {
                ...
                session.query(create_keyspace_query.as_str(), ()).await?;
                ...
                session.query(create_table_query.as_str(), ()).await?;
            ...
            pub async fn insert(session: &scylla::Session, item: #name) -> crate::result::Result<()> {
                let query = format!(
                    "INSERT INTO {}.{} (id, name, description, price, quantity) VALUES (?, ?, ?, ?, ?);",
                    #keyspace,
                    #table_name
                );
                session.query(query.as_str(), item).await.map(|_| ()).map_err(From::from)
            }
            ...
            pub async fn select_by_id(session: &scylla::Session, id: uuid::Uuid) -> crate::result::Result<Vec<#name>> {
                ...
                let rows = session.query(query.as_str(), (id,)).await?
                    .rows()
                    .unwrap_or_default();

                let result: Vec<#name> = rows
                    .into_typed::<#name>()
                    .map(|row| row.map_err(Into::into))
                    .collect::<Result<_, crate::error::OrmError>>()?;
